...
Crosses a :class:`~code_doc_monitor.inventory.SymbolInventory` (every file and
its extracted symbols) with a :class:`~code_doc_monitor.config.MonitorConfig`'s
...
:class:`~code_doc_monitor.extract.Symbol`) and adds no dependency (K0). Symbol
selection is **not re-implemented**: it reuses the private
:func:`code_doc_monitor.extract._select`, so whole-file (empty selectors),
