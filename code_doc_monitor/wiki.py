"""The ``cdmon wiki`` regeneration engine (EPIC R, R-08 — the close-out slice).
...
set, so ``cdmon wiki`` and ``cdmon wiki --check`` can never diverge. Each thunk
...
_DEMO_DIR = Path("demo")
_PKG_DIR = Path("code_doc_monitor")


def _known_modules() -> set[str]:
    """Top-level modules under ``code_doc_monitor`` (the catalog typo guard).

    Mirrors :func:`code_doc_monitor.cli._known_modules` so a feature naming a
