"""G-02 — `cdmon doctor`: an offline, read-only preflight (K1, K4, K10).

:func:`run_checks` answers one question — *is this repo wired up correctly enough
to run cdmon and report to its central system?* — WITHOUT touching the network
...
results; ``cdmon doctor`` prints them and exits 0 unless any is ``FAIL``.
...
def _check_documents(config: MonitorConfig, root: Path) -> Check:
    ...
    A missing doc *file* is PASS — ``cdmon new-doc`` / the heal can scaffold it.
