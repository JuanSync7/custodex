...
typed :class:`~code_doc_monitor.server.edits.ConfigEdit` tickets into a real,
...
1. apply the edits to the on-disk ``config/cdmon/*.yaml`` unit files (and the
   ...
   (:func:`~code_doc_monitor.config.upsert_document` /
   :func:`~code_doc_monitor.config.add_code_ref` / … ) +
   :func:`~code_doc_monitor.config.dump_unit_file`;
...
:class:`~code_doc_monitor.errors.CodeDocMonitorError` (K8); the clock is injected
...
def apply_record_fix(
    ...
    The offline, deterministic (K10), SCOPED counterpart of ``cdmon monitor
    ...
    1. loud K8 :class:`~code_doc_monitor.errors.CodeDocMonitorError` if the record
    ...
    3. find the matching :class:`DocumentSpec` (by ``doc_id``) to derive the
       ...
       :meth:`code_doc_monitor.monitor.Monitor.run` does for ``--apply`` (B-02/B-03
       — human regions are never authored; the guarantee is enforced at the heal
       write boundary);
    4. read the doc text BEFORE, call :func:`~code_doc_monitor.heal.apply_fix`
    ...
    if not index_path.is_file():
        raise CodeDocMonitorError(
            ...
            "(is this a cdmon repo with a dir-layout config?)"
        )
...
def apply_edits_to_disk(
    ...
    *,
    ...
    4. reload the bundle and, for each affected document, scaffold a missing doc
       ...
       human-owned regions (mirroring how ``cdmon monitor``/``new-doc`` derive
       ``preserve``/``modes``).
    ...
    :class:`~code_doc_monitor.errors.CodeDocMonitorError` (K8).
    """
    config_dir = local_path.joinpath(*_CONFIG_SUBDIR)
    index_path = config_dir / "index.yaml"
    if not index_path.is_file():
