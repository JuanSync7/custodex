...
dependency (stdlib ``ast`` + the existing :mod:`code_doc_monitor.traceability`
tag regex — K0); loud (:class:`code_doc_monitor.errors.CatalogError`) ONLY on a
...
def collect_tests(tests_root: Path) -> tuple[TestModule, ...]:
    ...
    loud :class:`code_doc_monitor.errors.CatalogError` (K8).
    """
...
def render_test_wiki_md(modules: tuple[TestModule, ...]) -> str:
    ...
    """
    lines: list[str] = [
        "# code-doc-monitor — test wiki",
        "",
        (
            "Generated from the tests' own docstrings + boundary directories — "
            "**do not hand-edit**. Run `cdmon wiki` (R-08) to regenerate."
        ),
        "",
