...
:mod:`code_doc_monitor.gitfetch` clone seam are reused VERBATIM — only the
...
* **GitHub App** — sign a short RS256 JWT with the App private key (``iss`` = the
  ...
  the single use of ``cryptography`` beyond :mod:`code_doc_monitor.secrets` — lazy
...
(K10). Every failure is a loud :class:`~code_doc_monitor.errors.TransportError` (K8).
"""
