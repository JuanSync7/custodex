"""code-doc-monitor — a standardized code→documentation drift monitor.
...
* invokes a pluggable **LLM backend** (headless Claude Code by default,
  swappable to the Anthropic API, or a deterministic mock for tests) to either
  **fix** the drift or **invalidate** it (a change irrelevant to the document's
  audience — e.g. a comment edit for a user guide),
