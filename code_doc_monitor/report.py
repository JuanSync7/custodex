...
:func:`code_doc_monitor.config.effective_coverage` to derive the scan scope from
the dir layout, then runs the EXACT same path the ``cdmon coverage`` CLI runs —
:func:`inventory.discover_files` → :func:`inventory.discover_symbols` →
:func:`coverage.resolve_coverage` — and projects the resulting
:class:`~code_doc_monitor.coverage.CoverageReport` into the report shape. So the
...
class RptSummary(BaseModel):
    ...
    :class:`~code_doc_monitor.coverage.CoverageReport` baskets. ``ignored_files``
