...
committed floor, every engine module is OWNED (an UNLISTED `custodex/**`
...
# `cdx coverage --fail-under` gate and tests/test_dogfood.py use (H-02).
...
def test_self_coverage_meets_committed_threshold() -> None:
    ...
    Mirrors the CI `cdx coverage --fail-under 95` gate (and test_dogfood) so the
...
# ---------------------------------------------------------------------------
# [H-01/H-04] An UNLISTED `custodex/**` module is detected as a GAP.
...
def test_unlisted_engine_module_is_detected_as_a_gap(tmp_path: Path) -> None:
    ...
    mutated), drops a NEW public module into `custodex/` WITHOUT adding it
    ...
    `custodex/**/*.py` (e.g. a narrowed `coverage.include`), the new
    ...
    new_mod = config_dir.parent.parent / "custodex" / "brand_new_unlisted.py"
    new_mod.write_text(
