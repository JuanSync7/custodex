# external-repo — a cdmon adopter example

A small, self-contained stand-in for *some other team's repo* that ADOPTS
code-doc-monitor. Unlike the dogfood (cdmon monitoring its own source) or the
...
To adopt cdmon in your own repo, copy a workflow from `templates/ci/` and run
`cdmon init --central <url> --repo-id <id>` to scaffold a config like this one.
