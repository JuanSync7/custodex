...
repo: custodex          # REQUIRED, repo id/name
...
- `root` is the repo root **relative to the directory the config lives in**
  ...
  `templates_root`, and `cdx rpt`) so the two layouts can never diverge.
- `units[].file` MUST list every `*.yaml` unit present in the dir, and every
  ...
  file is a loud `ConfigError` (K8). `cdx index` regenerates this list.
...
GENERATED by `cdx rpt` (and emitted during `cdx sync`). YAML + frontmatter:

```yaml
---
cdmon-report-version: "1.0.0"   # REQUIRED
kind: coverage                  # REQUIRED literal
repo: custodex          # REQUIRED
ref: main                       # branch/commit the report reflects
generated-by: cdx rpt         # REQUIRED provenance
---
summary:
...
undocumented:                   # files needing doc-sync, with where to declare them
  - path: custodex/agent/newnode.py
    suggested_unit: agent-workflow.yaml
    reason: "under dir-covered 'custodex/agent' and format '.py'"
```
...
- `cdx sync [--mode git|local] [--remote URL --repo-id ID]` — parse local
  `config/cdmon/`, and either POST to a central `--remote` or, with no remote,
  print the sync summary locally.
- `cdx index` — regenerate `config/cdmon/index.yaml` units list.
- `cdx rpt [--write]` — compute + print/write `config/cdmon/coverage.rpt`.
...
**Per-repo standalone view (epic L):** `cdx serve [--port 0] [--repo-id ID]`
...
- a README showing: `cdx check`, `cdx rpt --write`, `cdx serve` (standalone
...
documents) is cdmon's only self-config. `cdx check`/`coverage`/`rpt`
...
capability (`cdx init`, `examples/`, tested against a separate fixture — not
