...
regenerates the EPIC-R wikis so `cdx trace --fail-on-gap` and `cdx wiki
--check` stay green.

## Why
Every cdx feature must trace 1:1 to a demo AND a test (`cdx trace`). The new
...
4. **Trace 1:1.** `cdx trace --fail-on-gap` reports COMPLETE 199/199 (was 198):
   no feature missing a test/demo, no unknown refs.
5. **Wikis fresh.** `cdx wiki` regenerates; `cdx wiki --check` is clean.
...
- `cdx wiki` (regenerate all four wiki targets from their single sources).

## Test plan
- `cdx trace --fail-on-gap` → COMPLETE 199/199.
- `tests/integration/test_demo_traceability.py` (every feature has a demo) and the
  test-evidence checks stay green.
- `cdx wiki --check` clean; full offline pytest green.
