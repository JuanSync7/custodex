...
class BackendConfig(BaseModel):
    ...
    :class:`AgentConfig` and :mod:`custodex.agent`), whose *runtime* (the
...
class WaiverEntry(BaseModel):
    ...
    :mod:`custodex.inventory`). ``symbol`` names an exact symbol to
...
class CoverageConfig(BaseModel):
    ...
    ``include``/``exclude`` are the globs the A-05 ``cdx coverage`` CLI will
    pass to :func:`custodex.inventory.discover_files`; they default to
    the inventory's own defaults. ``waive`` carries intentional, justified
    documentation gaps folded into the coverage report
    (:class:`~custodex.coverage.CoverageReport`).
    """
...
# match your project, then run `cdx check` / `cdx monitor`.
...
# Should `cdx monitor` auto-apply FIX verdicts by default?
# (Override per-run with `monitor --apply` / `--no-apply`.)
apply_default: false

# Coverage scan scope + intentional, justified documentation gaps (A-04).
# Used by `cdx coverage`. `include`/`exclude` bound which code files are
...
def central_config_template(
    *,
    ...
    :func:`custodex.sinks.make_sink`'s HTTP requirements (``repo_id``
...
def _now() -> str:
    ...
    Mirrors :func:`custodex.monitor._default_now` and ``cli._now``; tests
...
def load_bundle(config_dir: Path) -> ConfigBundle:
    ...
    if unindexed:
        raise ConfigError(
            ...
            f"in {index_path}'s units; run `cdx index` to regenerate it"
        )
...
# ---------------------------------------------------------------------------
# CONFIG-V2 (N-02): index regeneration (`cdx index`).
#
...
def gitignore_to_globs(text: str) -> tuple[str, ...]:
    ...
    of :func:`custodex.inventory._translate` (``**/`` = zero-or-more
...
def resolve_repo_root(config_dir: Path, root: str) -> Path:
    ...
    ``templates_root``, and ``cdx rpt`` — so the dir layout and the single-file
