...
A backend turns one :class:`~custodex.drift.Drift` into a
...
def _context_ref_symbol_glance(path: str, repo_root: str | None) -> str:
    ...
    Reuses :func:`~custodex.extract.extract_file` (the same AST-only,
...
def _render_context_refs(req: FixRequest) -> str:
    ...
    Lists each :class:`~custodex.config.ContextRef` by ``path`` (+ its
...
class MockBackend:
    ...
    1. **Healable region drift -> FIX.** If ``drift.kind == REGION`` and the
       ...
       :func:`~custodex.blocks.expected_region` and return ``FIX``. This
    ...
    3. **Surface (fingerprint) HASH drift -> FIX (whole-doc).** Any remaining
       ...
       :func:`~custodex.heal.render_corrected` — exactly what an LLM would
