"""C-01 — the doc-patch producer behind ``cdx sync-pr`` (git-free, offline).

:func:`sync_pr` orchestrates AROUND an existing
:class:`~custodex.monitor.Monitor` to answer one question: *"what would
...
2. **Heal in place** — ``monitor.run(apply=True)``. Because this is the SAME pipeline
   ``cdx monitor --apply`` uses, B-02/B-03 region authority is honored automatically:
