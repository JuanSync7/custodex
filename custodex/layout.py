...
def _indexed_audiences(spec: DocumentSpec, config: MonitorConfig) -> set[str] | None:
    ...
    completeness rule matches what :func:`~custodex.index.render_index`
...
def _index_coverage_issues(config: MonitorConfig, root: Path) -> list[LayoutIssue]:
    ...
    for spec in config.documents:
        ...
        if not doc_path.is_file():
            continue  # existence is `cdx check`'s gate (MISSING_DOC)
        try:
...
def lint_config(config: MonitorConfig, config_dir: Path) -> list[LayoutIssue]:
    ...
    ``cdx check`` (existence is its gate); malformed front matter is reported
...
class RegionState(BaseModel):
    ...
    this re-reports state and never re-validates. ``cdx lint --modes`` prints
...
def region_states(
    ...
    :func:`custodex.blocks.known_region_ids(config.region_templates)`.
...
def config_region_states(config: MonitorConfig, config_dir: Path) -> list[RegionState]:
    ...
    The file-reading driver behind ``cdx lint --modes``: ``root = config_dir /
...
def scaffold_doc(
    ...
    code surface. The result passes both :func:`lint_doc` and ``cdx check``.

    ``include_body`` (P-01) must match ``MonitorConfig.fingerprint_body_tier`` so
    the scaffolded fingerprint agrees with what ``cdx check`` will compute —
