...
``__init__`` keeps that import lazy so ``import custodex`` core needs
nothing from here (K0, mirrors the ``[agent]`` extra).

**Unknown-repo policy (documented):** ingest never auto-registers. Registration is
explicit (``cdx register``, E-02); an ``/ingest`` (or records read) for an
...
_LOG = logging.getLogger("custodex.server")
...
def _wiki_dir() -> Path | None:
    ...
    beside ``dashboard/dist`` the SPA loader finds) so a single ``cdx``-server
    process can serve the rendered wikis on the same port as the API. Returns
    ``None`` when ``feature-doc/`` is absent (a non-cdx repo) so ``GET /wiki``
...
def _load_wiki_sections(wiki_dir: Path | None) -> list[dict[str, str]]:
    ...
    dependency-free :func:`custodex.build.render_markdown` (no new dep,
...
class SyncRequest(BaseModel):
    ...
    engine raises :class:`~custodex.errors.SyncError`), not a 422 — the
...
class DocStyleOptions(BaseModel):
    ...
    (:data:`custodex.docstyle.STYLE_CATEGORIES`), keyed by the
...
def _default_now() -> str:
    ...
    Mirrors :func:`custodex.config._now`. The config-sync route stamps
...
def _new_edit_id(repo_id: str, edit: ConfigEdit, now: str) -> str:
    ...
    Mirrors :func:`custodex.schema.new_record_id`: a sha1 prefix over the
...
def _scan_doc_styles(templates_root: Path) -> DocStyleOptions:
    ...
    (:data:`~custodex.docstyle.STYLE_CATEGORIES`), collect the ``*.md``
...
def _disk_editable_parts(
    ...
    * ``undocumented_files`` — the coverage gap: in-scope ``.py``/format files
      ...
      :func:`custodex.report.build_coverage_rpt` walks — never a re-scan.
    * ``ignored_files`` — the files the ignore/gitignore globs removed from the
      ...
      :func:`custodex.report._count_ignored`'s count). Sorted + capped to
      :data:`_IGNORED_FILES_CAP` (a note is logged when truncated).
    * ``unit_files`` — the on-disk unit STEMS (reusing
      :func:`custodex.config._scan_unit_files`'s glob, minus the reserved
...
def _resolve_provider_token(
    ...
    KEK raises :class:`~custodex.errors.SecretError` → the route's 500). Then,
    ...
    :func:`custodex.gitauth.mint_provider_token` (the hot token is never
    stored). A mint failure is a loud :class:`~custodex.errors.TransportError`.
    """
...
def create_app(
    ...
    *,
    ...
    the live ``cdx serve`` path serves the wikis with no extra wiring; a tmp dir
    ...
    default to the real implementations (a :class:`~custodex.gitfetch`
    ...
    returns a :class:`~custodex.pr.PRTransport`. ``token_exchange_http``
    (GIT-05, PHASE 2) is the injected leaf the App/OAuth short-lived-token mint uses
    (:mod:`custodex.gitauth`); default real, faked in tests so minting is
    offline.
    ...
    def root() -> object:
        ...
        return {
            "service": "custodex central server",
    ...
    def wiki() -> dict:
        ...
        engine's OWN dependency-free :func:`custodex.build.render_markdown`
        ...
        ``feature-doc/`` (a non-cdx repo) → ``{"sections": []}`` — a graceful
    ...
    def ownership_for(
        ...
        :func:`custodex.ownership.detect_orphans` — so marking someone
    ...
    def generate_config(
        ...
        :func:`~custodex.generate.apply_edits_to_disk` (the offline,
    ...
    def apply_record_fix_route(
        ...
        :func:`~custodex.generate.apply_record_fix` (offline, deterministic,
        SCOPED to the record's doc file), appends an ``accepted``
        :class:`ResolutionRecord` linked by ``record_id`` (mirroring ``POST
        /resolutions`` / ``cdx resolve``), re-runs ``run_sync`` and REPROJECTS the
        ...
        # `cdx resolve` / POST /resolutions — the SHARED ResolutionRecord, not a
...
def store_from_env() -> Store:
    ...
    head and a persistent :class:`~custodex.server.db.SqlStore` is returned —
