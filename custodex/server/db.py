...
the SAME :class:`~custodex.server.store.Store` Protocol — ``app.py`` and the
...
This module imports ``sqlalchemy`` (the ``[server]`` extra). ``import custodex``
...
class SqlStore:
    """A SQLAlchemy-backed :class:`~custodex.server.store.Store` (E-04).
