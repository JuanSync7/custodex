"""The per-repo STANDALONE app — ``cdx serve`` (L-01, no central access).
...
* :func:`build_standalone_store` builds an :class:`InMemoryStore` in which ONLY
  ...
  :func:`custodex.configsync.run_sync` for BOTH modes (local always; git
...
* :func:`build_standalone_app` wraps that store in :func:`create_app` and mounts
  ...
  (:func:`custodex.server.app._default_static_dir`) — the static-serving
...
import-safe builders; the ``cdx serve`` CLI keeps the uvicorn launch thin so
...
def resolve_repo_id(repo_root: Path, repo_id: str | None) -> str:
    ...
    same field ``cdx sync`` defaults from), falling back to the repo directory
...
def build_standalone_store(
    ...
    * **git** — best-effort: a brand-new repo with no ``main`` (or no commits)
      raises a :class:`~custodex.errors.SyncError`, which is swallowed so
...
def build_standalone_app(
    ...
    *,
    ...
    use (:func:`custodex.server.app._default_static_dir`) — so ``GET /``
