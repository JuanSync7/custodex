...
function defaultBaseUrl(): string {
  // An EXPLICIT `VITE_API_BASE` is honored verbatim — including "" which means
  // "same-origin root" (the combined single-port deploy, where FastAPI serves
  // both the API at `/` and this SPA). Only an UNSET var falls back to "/api"
  // (the dev-server case, where Vite proxies `/api` → the central server).
  const fromEnv = import.meta.env?.VITE_API_BASE;
  return typeof fromEnv === "string" ? fromEnv : "/api";
}
