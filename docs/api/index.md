...
| [spmirror](spmirror.md) | EPIC SP SharePoint mirror connector: list/fetch a library (a local `pull_sharepoint.py` mirror dir or the `rag-sharepoint-api` proxy), convert each file to deterministic text (`docx-text` container-churn- invariant, `passthrough`; a lossless doc2md plugs into the same registry), and write the governed mirror under `dest` — preserving every engine-stamped `cdm:` baseline across re-syncs (`spmirror`). |
