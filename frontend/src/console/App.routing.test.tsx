...
describe("App routing", () => {
  ...
  it("links to the native Astro wiki via the Wiki nav link (real navigation, not a client route)", async () => {
    vi.stubGlobal("fetch", stubFetch());
    ...
    // EPIC ASTRO: the wikis are native `/wiki/*` Astro pages now, so the Wiki nav
    // item is a REAL anchor (a full-page navigation that leaves the SPA), present
    // before any click — NOT a client (#/wiki) route. We assert the href; the
    // page content itself is an Astro page (built from feature-doc/*.md).
    const wikiLink = await screen.findByRole("link", { name: /wiki/i });
    expect(wikiLink).toHaveAttribute("href", "/wiki/features");
  });
